...
function TCDriver_GetActor(act){
...
function TCDriver_SetState (lrs, activityId, stateKey, stateVal, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
        var url = lrs.endpoint + "TCAPI/activities/state?activityId=<activity ID>&agent=<actor>&stateId=<statekey>";
        ...
        if (lrs.registration) {
        	url += "&registration=" + encodeURIComponent(lrs.registration);
                     ...
                     //lrs, url, method, data,                  auth,       callback, ignore404, extraHeaders
        var xtra = new Object();
        xtra['If-None-Match'] = "thismightbeabug";
        XHR_request(lrs,url, "PUT", JSON.stringify(stateVal), lrs.auth, callback, false, xtra);
    }
}
...
function TCDriver_GetStatements (lrs,sendActor,verb,activityId, callback) {
    if (lrs.endpoint != undefined && lrs.endpoint != "" && lrs.auth != undefined && lrs.auth != ""){
        ...
        if (verb != null){
            url += "&verb=" + encodeURIComponent(verb['id']);
        }
        if (activityId != null){
